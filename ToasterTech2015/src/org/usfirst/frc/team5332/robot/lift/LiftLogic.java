...
/*
 * Logic to control lift and keep track of lift state.
 */

public class LiftLogic extends
		SystemLogic<LiftSystem> {
	/*
	 * Lift Height is an enum to keep track of lift state and perform
	 * calculations to determine sequential heights, and the distance between
	 * them
	 */
	public enum LiftHeight {
		Min(0), OneTote(1), TwoTote(2), ThreeTote(
				3), Max(4), Transition(-1), NONE(
				-1);
	...
	public LiftHeight getLastHeight() {
		...
		if ((temp == LiftHeight.NONE)
				|| (temp == LiftHeight.Transition)) {
			return lastHeight;
		} else {
			return temp;
		}
	...
	public LiftHeight getLiftHeight() {
		/*
		 * Voltage: 5- Highest Magnetic Reed Switch value 4 3
		 * 2sensorReturnedValue 1- Lowest Magnetic Reed Switch value 0- No
		 * Magnetic Reed Switch value, is in TRANSITION
		 */
		LiftSensor.switchnum sensorReturnedValue;
		sensorReturnedValue = system
				.getSwitchValue();
		...
		case none:
			...
			if (system.getMoving())
				return LiftHeight.Transition; // Because it is not reading
												// anything, but is moving, it
												// is in transition
			else
